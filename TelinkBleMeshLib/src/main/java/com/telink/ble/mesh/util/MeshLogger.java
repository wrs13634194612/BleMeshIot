...
public class MeshLogger {
    ...
    public static final String DEFAULT_TAG = "wrsSIG-Mesh";
...
//    public static Logger logger = Logger.getLogger(GLOBAL_TAG);
    ...
    public static void log(String logMessage, String tag) {
        log(logMessage, "155", DEFAULT_LEVEL);
    }

    public static void log(String logMessage, int level) {
        log(logMessage, "156", level);
    }
    ...
    public static void log(String logMessage, String tag, int level) {
        if (print) {
            switch (level) {
                case LEVEL_VERBOSE:
                    Log.v("wts1_tag", logMessage);
                    break;
                case LEVEL_INFO:
                    Log.i("wts2_tag", logMessage);
                    break;

                case LEVEL_WARN:
                    Log.w("wts3_tag", logMessage);
                    break;

                case LEVEL_ERROR:
                    Log.e("wts4_tag", logMessage);
                    break;

                case LEVEL_DEBUG:
                default:
                    Log.d("wts5_tag", logMessage+","+tag+","+level);
            }
        }
