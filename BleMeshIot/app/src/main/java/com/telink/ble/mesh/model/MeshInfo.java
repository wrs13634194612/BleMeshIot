...
//        meshInfo.networkKey = NET_KEY;
        ...
        for (int i = 0; i < KEY_COUNT; i++) {
            meshInfo.meshNetKeyList.add(new MeshNetKey(NET_KEY_NAMES[i], i, MeshUtils.generateRandomBytes("e5e5e5e50d0d0d0d0d0d0d3131313131")));
            meshInfo.appKeyList.add(new MeshAppKey(APP_KEY_NAMES[i],
                    i, APP_KEY_VAL, i));
        }

        meshInfo.ivIndex = 305419896;
...
//        meshInfo.provisionerUUID = SharedPreferenceHelper.getLocalUUID(context);
        ...
        for (int i = 0; i < 8; i++) {
            group = new GroupInfo();
            group.address = i | 0xC100;
            group.name = groupNames[i];
            meshInfo.groups.add(group);
        }

        Log.e("wts4_tag", "101logMessage");
